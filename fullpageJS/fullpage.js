	...
	function initialise(containerSelector, options) {
		...
		function prepareDom() {
			...
			if (options.fixedElements && options.css3) {
				$(options.fixedElements).forEach(function (
					item
				) {
		...
		function styleSlides(section, slides, numSlides) {
			...
			if (
				startingSlide != null &&
				(index($(SECTION_ACTIVE_SEL), SECTION_SEL) !==
					0 ||
					(index(
						$(SECTION_ACTIVE_SEL),
						SECTION_SEL
		...
		function scrollHandler() {
			...
			if (!options.autoScrolling || options.scrollBar) {
				...
				var screen_mid =
					currentScroll + getWindowHeight() / 2.0;
				var isAtBottom =
					$body.offsetHeight -
						getWindowHeight() ===
					currentScroll;
		...
		function isSectionInViewport(el) {
			...
			var isTopInView =
				top + pixelOffset < windowsHeight && top > 0;
			var isBottomInView =
				bottom > pixelOffset && bottom < windowsHeight;
        ...
        */
		function touchMoveHandler(e) {
			...
			if (isReallyTouch(e)) {
				...
				if (
					$(SLIDES_WRAPPER_SEL, activeSection)
						.length &&
		...
		function getDestinationPosition(element) {
			...
			var sectionBottom =
				position - windowsHeight + elementHeight;
		...
		function lazyLoadOthers() {
			...
			$(SECTION_SEL + ":not(" + ACTIVE_SEL + ")").forEach(
				function (section) {
		...
		function playMedia(destiny) {
			...
			//youtube videos
			$('iframe[src*="youtube.com/embed/"]', panel).forEach(
				function (element) {
					if (
		...
		function performHorizontalMove(slides, v, fireCallback) {
			...
			if (options.css3) {
				...
				FP.test.left[v.sectionIndex] = Math.round(
					destinyPos.left
				);
		...
		function responsive() {
			var widthLimit =
				options.responsive || options.responsiveWidth; //backwards compatiblity
		...
		function scrollSlider(slide) {
			if (slide != null) {
				landscapeScroll(
					closest(slide, SLIDES_WRAPPER_SEL),
					slide
				);
			}
		}
		...
		function setIsScrollAllowed(value, direction, type) {
			...
			//all directions?
				...
				Object.keys(isScrollAllowed[type]).forEach(
					function (key) {
		...
		function destroyStructure() {
			...
			$(SECTION_SEL + ", " + SLIDE_SEL).forEach(function (
				item
			) {
				if (
